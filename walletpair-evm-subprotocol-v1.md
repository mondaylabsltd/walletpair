...
5. Transaction type and fee field consistency:
   ...
   - If `tx.type` is `"0x4"` (EIP-7702): `gasPrice` MUST NOT be
     ...
     (d) If the recovered authority is an account managed by this
         ...
         default. A wallet MAY allow this only if the authorization
         was generated by the wallet's own controlled flow (e.g.,
         smart account setup) AND the delegation target is on a
         wallet-maintained allowlist of audited contracts.
