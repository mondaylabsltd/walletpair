...
Creates the channel, accepts the wallet, sends requests. To reject a
wallet, the dApp sends `close` with an appropriate reason (e.g.,
`user_rejected` or `unsupported_capability`). There is no separate
reject message.
...
Present in `req`, `res`, and `evt`.

- **Requests:** Unique per `req`. The wallet copies the same `id` into
  the matching `res`. SHOULD be UUID v4 or equivalent entropy.
- **Events:** Unique per `evt`. Generated by the wallet. SHOULD be
  UUID v4 or equivalent entropy.
...
- `_method` is required in `req`. If missing, the wallet MUST reply
  with an error `res` (`_ok: false`, code `invalid_params`).
- `_event` is required in `evt`. If missing, the dApp SHOULD ignore
  the event and log the error. Do NOT close the channel.
- `_ok` is required in all `res`. If missing, the dApp SHOULD treat
  the response as an error and log it. Do NOT close the channel.
- `_result` may be any JSON value including `null`.
- Every `req`, `res`, and `evt` MUST carry `sealed` after
  `ready.connected`. A `req` without `sealed` MUST be rejected with
  an error `res` (code `protocol_error`). A `res` or `evt` without
  `sealed` MUST be ignored and logged.

### 4.4 Error Codes

Error codes appear in the `code` field of error responses
(`_ok: false`). They are distinct from close/terminate reasons
(Section 12.3).

| Code | Meaning |
|------|---------|
| `invalid_params` | Missing or malformed parameters (e.g., `_method` absent in `req`). |
...
with reason `normal` and require fresh pairing. The limit is `2^31`
...
1. If `req` uses a method not in `capabilities.methods`, reply with
   error `res` (code `unsupported_method`). Do NOT close the channel.
2. If `req` targets a chain not in `capabilities.chains`, reply with
   error `res` (code `unsupported_chain`). Do NOT close the channel.
...
1. `res.id` MUST equal the matching `req.id`.
2. Each request receives exactly one response.
3. See Section 4.3 for decrypted payload format.
...
1. Events do not require a response.
2. Event ordering follows transport order while connected.
3. No event replay guarantee after reconnect. The dApp SHOULD call a
...
If the previous channel has already been cleaned up (expired or
closed), the relay creates a new channel with the same `ch`. If the
...
negotiated during initial pairing. The dApp matches `from` against its
persisted `peer_public_key` and auto-sends `accept`.
...
11. A peer MUST NOT have more than 32 pending (unanswered) requests per
    ...
    "Too many pending requests"). Do NOT close the channel. The dApp
...
16. **Session expiry.** A channel MUST have a maximum session lifetime
    (recommended default: 24 hours from `ready.connected`). On expiry,
    close with reason `timeout` and require fresh pairing — reconnect
    MUST NOT be used after session expiry. The relay SHOULD also enforce
...
blockchain ecosystem. A sub-protocol MUST cover the following topics.
How they are organized into sections is up to the author. See
`walletpair-evm-subprotocol-v1.md` as a reference implementation.

| Topic | What to define |
|-------|----------------|
| **Namespace and version** | Identifier (e.g., `evm`), version integer, CAIP-2 prefix (e.g., `eip155`). |
| **Chain identification** | CAIP-2 format, chain ID encoding in params, format conversion rules. |
| **Account identification** | Address format, length, checksum, forbidden values. |
| **Capabilities** | Registry of methods, events, and chains. Which methods are required vs. optional. |
| **Data encoding** | How binary data, integers, addresses, transactions, and signatures are encoded within `sealed` JSON. |
| **Methods** | For each method: params schema, result schema, validation rules, user confirmation requirements. MUST include at least one account query method and one signing method. |
| **Events** | For each event: data schema, trigger conditions, dApp handling. SHOULD include `accountsChanged`, `chainChanged` (if multi-chain), and `disconnect`. |
| **Error codes** | All error codes with meaning. Include both standard codes (from main protocol Section 4.4) and sub-protocol-specific codes. |
| **Security requirements** | User confirmation matrix, blind signing policy, cross-chain replay rules, high-risk pattern detection, session isolation. |

Sub-protocols MAY add additional topics as needed (e.g., RPC method
routing, transaction type capabilities, provider adapter mapping).
