...
sides. These `ready` messages are generated by the adapter itself —
neither the dApp nor the wallet sends them.
...
| `method`       | `req`                                                                                    | Wallet method name. MUST be `"encrypted"` unless both peers opt out of privacy mode (§7.4). Visible to relay. |
...
| `event`        | `evt`                                                                                    | Event name. MUST be `"encrypted"` unless both peers opt out of privacy mode (§7.4). Visible to relay.         |
